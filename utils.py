...
# gonna refactor Dataset into a more generic class
class Dataset_16(object):
    def __init__(self, path, cache_path = './cache_16'):
        if os.path.isfile(cache_path):
            print 'Loading cache from %s' % cache_path
            imgs, labels = load(cache_path)
        else:
...
# actually gonna refactor after this
class Dataset_64(object):
    def __init__(self, path, cache_path = './cache_64'):
        if os.path.isfile(cache_path):
            print 'Loading cache from %s' % cache_path
            imgs, labels = load(cache_path)
        else:
        ...
        self.training_img = imgs[range(0, l, 2)]
        self.training_label = labels[range(0, l, 2)]
        self.testing_img = imgs[range(1, l, 4)]
        self.testing_label = labels[range(1, l, 4)]
        self.real_testing_img = imgs[range(3, l, 4)]
        self.real_testing_label = labels[range(3, l, 4)]
